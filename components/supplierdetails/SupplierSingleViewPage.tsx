...
export default function SupplierSingleViewPage() {
...
const paidTrips = trips.filter(
  ...
  const fetchTrips = async () => {
   const [{ data ,error }, { data: supplierData }] =
        await Promise.all([
          supabase
            .from("tripsTable")
            .select(`
      ...
      (data || []).map((trip) => {
        const startDate = new Date(
         ...
         const supplier = supplierData?.find(
            (p) => p.supplierID === trip.supplierID
          )
        ...
        return {
          ...trip,
          partyName: trip.partiesTable
            ? trip.partiesTable.partyName
            : "N/A",
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">
        Details for{" "}
        {trips.length > 0 && trips[0].supplierName
          ? `${trips[0].supplierName}`
          : "Supplier Details"}
      </h1>
