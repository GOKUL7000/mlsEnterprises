...
export default function PartiesSingleViewPage() {
  const params = useParams()
  const partiesID = params.id
...
const paidTrips = trips.filter(
  ...
  const fetchTrips = async () => {
   const [{ data ,error }, { data: partyData }] =
        await Promise.all([
          supabase
            .from("tripsTable")
            .select(`
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">
        Details for{" "}
        {trips.length > 0 && trips[0].partyName
          ? `${trips[0].partyName}`
          : "Party Details"}
      </h1>
