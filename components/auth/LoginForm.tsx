...
//   const { data: loginTable, error } = await supabase
//     .from('loginTable')
//     .select('*')
//     .eq('username', email)
//     .single();
...
//       return;
//     }
//     const isValid = await bcrypt.compare(password, loginTable.password);
...
//   };
  const handleLogin = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const { data: loginTable, error: loginError } = await supabase
          .from("loginTable")
          .select("*")
          .eq("username", email)
          .single()

        if (loginError || !loginTable) {
    ...
    <CardContent className="p-8">
      ...
      <p className="text-center text-xs text-muted-foreground mt-8">
        ...
        <a
          ...
          >
            Developed by Altix Software Solutions
          </a>
      </p>
    </CardContent>
  )
